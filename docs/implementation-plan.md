...
**Status:** Historical planning draft. For current shared isolation semantics,
Linux behavior, and setup requirements, see
[`docs/axis-isolation-contract.md`](axis-isolation-contract.md) and
[`docs/linux-setup.md`](linux-setup.md).
**Date:** April 2026
...
### Historical Implementation Status (2026-04-09)
...
A veth pair (10.200.0.1 host ↔ 10.200.0.2 sandbox) routes all sandbox traffic through an HTTP CONNECT proxy. For endpoint policies with `binaries`, Linux native proxy launch uses seccomp-notify to observe `connect(2)` at the kernel boundary: the supervisor duplicates the child socket while the child is blocked, performs the connect, records the resulting tuple and executable fingerprint, and the proxy consumes that record before OPA evaluation. `/proc/net/tcp` → PID → `/proc/[pid]/exe` remains a fail-closed fallback only when the policy has no binary allowlist. For allowed connections, optional L7 inspection TLS-terminates the stream (ephemeral per-sandbox CA), parses HTTP requests, and enforces method/path rules. Host-boundary credential injection lets approved provider routes receive API keys without exposing them in the agent's environment.
...
Landlock ABI is available on modern Linux kernels. Current AXIS behavior is to
use Landlock when it can enforce the policy, use an explicit supported fallback
such as bubblewrap for block mode when that fallback preserves the requested
semantics, or fail closed before spawning user code.
