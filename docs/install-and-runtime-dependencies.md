...
| --- | --- | --- |
| AXIS-native process sandbox | Platform OS sandbox support: Landlock and seccomp on Linux, AppContainer/Job Object support on Windows, and Seatbelt support on macOS. | Default user install. If a requested policy cannot be enforced by available OS features, AXIS rejects before spawn. |
| MXC process sandbox | Packaged MXC executor for the selected platform backend. Bubblewrap is an optional Linux process backend dependency when that backend is selected. | Default user install for packaged non-privileged executors. Optional tools are discovered safely and are not installed as part of normal tests. |
