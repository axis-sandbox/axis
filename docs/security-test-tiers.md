...
backend planning, VM backend planning, and the MXC fake executor/dry-run paths
by default. The MXC command intentionally skips tests named with the `gated_`
prefix so real-runtime and privileged proofs do not become hidden host-specific requirements.
