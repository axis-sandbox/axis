...
def verify_security_workflow(workflow: dict[str, Any]) -> None:
    ...
    expected_names = {
        "distribution-notices": "Distribution notices",
    ...
    expected_runtime = {
        "distribution-notices": ("ubuntu-24.04", 20, None),
        "codeql": (
            "ubuntu-24.04",
            30,
    ...
    require_exact_gate_step(
        ...
        {
            "NOTICES_RESULT": "${{ needs.distribution-notices.result }}",
        ...
        """set -euo pipefail
for result in "$NOTICES_RESULT" "$CODEQL_RESULT" "$SWIFT_RESULT" \\
  "$AUDIT_RESULT" "$NUGET_RESULT" "$SAST_RESULT" \\
  "$SECRETS_RESULT" "$ACTIONS_RESULT"; do
  test "$result" = success
done
""",
    )
    ...
    notices = require_job(workflow, "distribution-notices")
    verify_notice_commands(
        notices,
        expected_mxc_ref=False,
        rust_toolchain="1.95.0",
        label="security notice verification",
    )
