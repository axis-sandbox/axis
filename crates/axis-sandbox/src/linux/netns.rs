...
pub fn create_netns(sandbox_name: &str, proxy_port: u16) -> Result<String, String> {
    ...
    match strategy {
        NetnsStrategy::IpNetns => {
            match create_with_ip_netns(&ns_name, &veth_host, &veth_sandbox, proxy_port) {
                Ok(name) => Ok(name),
                Err(e) => {
                    let _ = destroy_netns(&ns_name);
                    Err(e)
                }
            }
        }
...
fn create_with_ip_netns(
    ...
    run_cmd(
        "ip",
        &[
            "netns", "exec", ns_name, "iptables", "-A", "OUTPUT", "-o", "lo", "-j", "ACCEPT",
        ],
    )?;
    ...
    run_cmd(
        "ip",
        &[
            "netns", "exec", ns_name, "iptables", "-A", "OUTPUT", "-j", "REJECT",
        ],
    )?;
...
pub fn destroy_netns(ns_name: &str) -> Result<(), String> {
    ...
    if let Err(e) = run_cmd("ip", &["link", "del", &veth_host]) {
        tracing::debug!(
            "netns: host veth '{veth_host}' already absent or could not be deleted: {e}"
        );
    }
...
fn veth_host_name(sandbox_name: &str) -> String {
    format!("axh-{}", &sandbox_name[..sandbox_name.len().min(8)])
}

fn veth_sandbox_name(sandbox_name: &str) -> String {
    format!("axs-{}", &sandbox_name[..sandbox_name.len().min(8)])
}
...
mod tests {
    ...
    fn veth_names_are_derived_from_sandbox_id_prefix() {
        assert_eq!(veth_host_name("1234567890"), "axh-12345678");
        assert_eq!(veth_sandbox_name("1234567890"), "axs-12345678");
    }
    ...
    fn destroy_derives_sandbox_name_from_axis_namespace() {
        ...
        assert_eq!(veth_host_name(sandbox_name), "axh-12345678");
    }
}
